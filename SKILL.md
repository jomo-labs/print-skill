...
Follow `references/assembly.md` exactly: copy `assets/page_template.html` to
`<slugified-title>.html`, then make the anchored insertions — nested-sheet CSS
(two-sheet only), font `<link>`, `custom_css` into
`<style id="content-overrides">`, the `<body>` data attributes
(`data-mp-paper`, `data-mp-live-edit`) when paper/live_edit are set, and
finally replace `<!-- CONTENT -->` with your content. Copy the template and
shell assets with `cp` — never retype them.
...
Before serving or editing an existing page, refresh its shell assets:
`cp -f <skill-dir>/assets/shell/* <outdir>/shell/` (always safe — nobody edits
them in place; a stale copy from an older skill version breaks pages opened as
`file:`, and the server serves the skill's current shell regardless). Pages
generated by older skill versions may still contain baked-in chrome markup or
injected `applySize`/`setLiveEditSupported` script lines — leave them alone:
the current shell removes and replaces old chrome at load, and the next
browser-edit save cleanses the file automatically.
