...
`assets/shell/chrome-host.css`, `assets/shell/shell.js`,
`assets/shell/chat.js` — toolbar, paper-size switching, edit mode, the chat
...
`chrome-host.css` + `shell.js` + `chat.js` — toolbar, edit mode, chat panel)
...
its own: whatever the page's CSS does, the chrome looks the same. (Pages generated by older skill versions may link `shell/*` files
...
There is deliberately no live-edit attribute. Whether the page can chat with
a model is not a property of the document — it is whether the page is being
