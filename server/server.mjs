...
// its relative shell/ and font references resolve, then rendered by
// render.mjs.
//
...
const ROOT = path.resolve(argValue("--dir", process.cwd()));
...
async function handleRenderPdf(req, res) {
  ...
  try {
    ...
    const pdf = await renderPdf(`http://127.0.0.1:${PORT}/${temp}`);
    res.writeHead(200, {
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(`print-skill server: http://127.0.0.1:${PORT}  (serving ${ROOT})`);
});
